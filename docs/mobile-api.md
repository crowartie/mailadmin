...
- **Раздача — GitHub Releases** (подписанный APK), магазины — когда-нибудь потом. Приложение само
  проверяет последний выпуск и предлагает обновиться (установка не из магазина обновляется только так).
...
оставлено веб-почте: включение 2FA по QR (в приложении — по ключу), подпись с картинками (в приложении — текстом).
...
- Push пока не подключён (нет проекта Firebase): фоновая проверка раз в 15 минут.
- Выпуски — GitHub Releases, метка `mobile-vX.Y.Z` (`.github/workflows/mobile-release.yml`).
