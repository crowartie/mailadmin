        ...
        <aside class="drawer" role="dialog" aria-modal="true" :aria-label="mailbox.username" style="width: 760px">
            ...
            <form id="mailbox-form" class="drawer__body" @submit.prevent="submit">
                ...
                <template v-if="activeTab === 'access'">
                    ...
                    <p class="hint">Коллега увидит открытую папку у себя в разделе «Общие папки». Читатель только смотрит, редактор может перекладывать и удалять письма, владелец (только по «Входящим») ещё и пишет от имени этого ящика. Редактору и владельцу «Входящих» открываются и системные папки ящика: «Спам», «Корзина», «Отправленные»… Хозяин ящика делает то же самое сам в веб-почте (правой кнопкой по папке → «Общий доступ»).</p>
