...
/**
 * Тон кружка с инициалами по адресу: 1…6, у человека всегда один и тот же, так что письма
 * от одного отправителя видно, не читая имени. Тонов шесть (--av-1…6 в _variables.scss),
 * а не произвольный hue: пастель по углу цветового круга на тёплом фоне гаммы читалась
 * плохо, а шесть глубоких тонов под белые буквы подобраны по контрасту.
 */
export function avatarTone(mail) {
    const s = String(mail || '').toLowerCase();
    let n = 0;
    for (let i = 0; i < s.length; i++) n = (n * 31 + s.charCodeAt(i)) % 6;
    return n + 1;
}

/** Готовое значение для style: { '--av': avatarColor(mail) }. */
export function avatarColor(mail) {
    return `var(--av-${avatarTone(mail)})`;
}
