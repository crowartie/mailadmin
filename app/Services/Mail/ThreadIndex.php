...
{
    /**
     * Папки, где цепочки не ищем.
     *
     * Раньше сюда входили «Спам» и «Корзина», и переписка получалась с дырами: ответ,
     * который сервер по ошибке счёл спамом, в цепочке не появлялся вовсе, и человек
     * решал, что ему не ответили. Теперь они в индексе, а в переписке помечены, откуда.
     * Общие папки коллег остаются снаружи: их индексирует владелец, и повторять эту
     * работу в ящике каждого, кому они открыты, незачем.
     */
    private const SKIP_ROLES = ['shared'];
