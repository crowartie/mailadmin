...
class Calendars
{
    ...
    /**
     * Отписаться от чужого общего календаря. Для приложения это отдельное действие:
     * deleteCalendar на чужом календаре тоже лишь отписывает, но у пользователя в руках
     * кнопка «Удалить», и путать её с «Отписаться» нельзя — свой календарь этот метод не тронет.
     */
    public function unsubscribe(string $user, string $uri): void
    {
        $c = $this->calendar($user, $uri);
        if ($c['kind'] !== 'shared') {
            throw new DavException('Это не чужой календарь: отписаться можно только от того, чем поделился коллега', 403);
        }
        // sabre на DELETE чужого экземпляра удаляет только строку доступа этого сотрудника.
        $this->a->dav($user, 'DELETE', "calendars/{$user}/{$uri}/");
    }
